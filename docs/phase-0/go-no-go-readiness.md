...
| GPUI settings            | GPUI 0.2.2 默认 shader、`.app`、主题、文本状态 contract、runtime bridge、AX/UIA 最小 tree/action 和窗口重开已通过；macOS 与 Windows modal dialog/焦点/语义子树通过；macOS AX 可观察 loading/error/retry/revision 恢复                                                                                                   | Windows busy/error/retry UIA CI；真实中文 IME、VoiceOver/Narrator 宣读、tooltip 真实 hover/朗读、完整菜单和目标 DPI    | P0 blocker；ADR-0009 保持 Proposed                        |
