...
| Cubism ABI/model         | 三个预置 model3/cdi3、6 个 motion3、15 个 exp3、13 个本机历史 physics3 与合成 pose3/userdata3 已由 Rust 静态 parser 验证；macOS arm64 真实 r.5 binding/Core 又完成三个预置 Moc 各 100 次 consistency/revive/model/update、全部 drawable 数组、r.5 offscreen 零长度路径与 `leaks` 0-byte 结果，详见 `cubism-core-r5-probe.md`                                                                                                                                                                                                         | Windows x64/macOS x64 原生 ABI、非零 offscreen fixture、产品 safe owner、Framework 求值、授权 physics/pose fixture、D3D11/Metal 模型绘制                            | P0 blocker；arm64 Core 数组读取不等同完整 Live2D/renderer |
