...
| Cubism ABI/model         | 三个预置 model3/cdi3、6 个 motion3 和 15 个 exp3 已由 Rust 强类型 parser 验证；cdi3 计数和 legacy Web Core baseline 一致                                                                                 | Native R5 binding、ABI、Core ID 对照、Moc/Model owner、drawable、Framework 求值、授权 physics/pose fixture、100 次切换 | P0 blocker；资源结构验证不替代 Core/求值             |
