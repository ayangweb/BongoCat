...
3. GPUI 尚缺真实 IME、VoiceOver/Narrator、tooltip 真实 hover/朗读、完整菜单和目标 DPI 矩阵；
...
| GPUI settings            | GPUI 0.2.2 默认 shader、`.app`、主题、文本状态 contract、runtime bridge、AX/UIA 最小 tree/action 和窗口重开已通过；双平台 modal dialog/焦点/语义子树通过；macOS AX 与 Windows UIA 均可观察 loading/error/retry，其中 Windows push run `33291750411`、job `99204478369` 与 PR run `33291751558`、job `99204481348` 已通过 revision 2 恢复门禁 | runner 托管 UIA client 缺少 AriaProperties 标识，`busy=true` 投影未验证；真实 IME、VoiceOver/Narrator、tooltip hover/朗读、完整菜单和目标 DPI | P0 blocker；ADR-0009 保持 Proposed                        |
