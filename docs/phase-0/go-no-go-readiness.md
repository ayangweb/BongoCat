...
| GPUI settings            | GPUI 0.2.2 默认 shader、`.app`、主题、文本状态 contract、runtime bridge、AX/UIA 最小 tree/action 和窗口重开已通过；双平台 modal dialog/焦点/语义子树通过；macOS AX 可观察 loading/error/retry，Windows UIA 可观察 loading value                                                                                         | Windows `busy=true` 未通过 UIA AriaProperties 投影，error/retry CI 待完成；真实 IME、VoiceOver/Narrator、tooltip hover/朗读、完整菜单和目标 DPI | P0 blocker；ADR-0009 保持 Proposed                        |
