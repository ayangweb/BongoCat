...
| Cubism ABI/model         | 三个预置 model3/cdi3、6 个 motion3、15 个 exp3 与 13 个本机历史 physics3 已由 Rust 静态 parser 验证；physics 只保留匿名聚合，cdi3 计数和 legacy Core baseline 一致                                       | Native R5 binding、ABI、Core ID 对照、Moc/Model owner、drawable、Framework 求值、授权 physics/pose fixture、100 次切换 | P0 blocker；历史结构验证不替代授权 fixture/Core/求值 |
