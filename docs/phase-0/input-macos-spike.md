...
状态：权限/tap 生命周期 contract、listen-only CGEventTap、可靠 callback 队列、cursor/gamepad axis latest-value 通道、键盘/鼠标按钮周期校正、GameController owner、真实 keyboard/modifier/mouse callback release 丢弃后的恢复、受控 disable 恢复、NSWorkspace 生命周期 Reset 和 callback shutdown smoke 已通过；权限矩阵、真实系统生命周期、系统自然 timeout 和物理键鼠/手柄矩阵仍待验证
日期：2026-08-29
...
CoreGraphics binding 仅存在于 macOS target dependency；非 macOS 构建会输出 skipped，不引入跨平台 API。`--tap-ms <milliseconds>` 会在专用线程/run loop 上创建 listen-only `CGEventTap`，只统计事件类型、可靠队列和 cursor 合并诊断，不记录具体键值或坐标。键盘事件在 queue 中保留 keycode/repeat；鼠标 down/up 保留 CoreGraphics 的 0–31 号 button identity，不再把左、右、中和侧键压成同一个事件。`FlagsChanged` 在 callback 中用事件自身 flags、左右 modifier keycode 和 callback-local decoder set 冻结 pressed/released 方向，避免 consumer 稍后 drain 时用较新的全局状态误判旧边沿，也处理左右同类 modifier 同时按下时聚合 flag 的歧义；decoder set 不作为 runtime pressed state，并随 Reset 清空，无法识别的 modifier 会排队安全 Reset 并增加匿名计数。run-loop consumer 分别维护键盘和鼠标候选 pressed set；每 `250 ms` 只查询当前候选 key/button，经 `CGEventSourceKeyState`/`CGEventSourceButtonState` 连续 `2` 次缺失才形成 reconciled release。`Reset`、tap shutdown 和 queue overflow 同时清空两类候选。`--cycles <count>` 可重复创建、运行、禁用并销毁 tap，0 会被拒绝；`--summary-only` 省略逐 cycle 行但保留严格校验和聚合结果。任一 cycle 创建失败、未恢复 enabled、callback panic、未知 modifier、队列 overflow/close 后事件、cursor accounting 不完整、observer 数量不匹配或注入语义失败都会以非零状态退出，不再只打印错误。`--key-state <macOS-keycode>` 和 `--button-state <0..31>` 分别对单个候选执行系统状态查询，只输出 checked/still-pressed/released 数量。默认仍不会自动创建 tap。
...
`--release-loss-kind keyboard|modifier|mouse` 只能和 `--tap-ms` 一起用于 release 校正闭环；旧的 `--inject-release-loss` 保留为 keyboard 别名。probe 会分别 preflight listen-only Input Monitoring 和 synthetic Post Event/Accessibility 权限；后者未授予时以 `PostEventPermissionDenied` 明确失败，不触发权限请求。权限齐备后通过 private `CGEventSource` 向 session event tap 投递一对 keyboard、left Shift `FlagsChanged` 或匿名高编号 mouse button down/up；listen-only callback 必须收到两个真实 CoreGraphics 事件，但 callback 到 consumer 的边界会故意丢弃一次对应 release。consumer 保留 down 建立的候选，随后只通过两次系统状态缺失确认清除它。命令会按类型断言投递数、callback 计数、丢弃数、校正数和 shutdown 前候选数，不输出 keycode、button 或坐标。它验证真实 event tap callback 到校正实现的闭环，但不能代替物理键鼠、系统丢事件或 TCC 变化实测。
...
cargo run --manifest-path spikes/input-macos/Cargo.toml --locked --release -- --tap-ms 600 --cycles 20 --summary-only --release-loss-kind modifier
cargo run --manifest-path spikes/input-macos/Cargo.toml --locked --release -- --tap-ms 600 --cycles 20 --summary-only --release-loss-kind mouse
...
2026-08-30 同一设备、系统和权限条件下将 release-loss 扩展到 callback-time modifier flags 与 mouse button 字段。keyboard、modifier、mouse 三种单 cycle 均收到两次对应 callback、故意丢弃一次 release、经 3 次调度中的连续 2 次缺失确认得到 `reconciled_releases=1 pressed_candidates_before_shutdown=0`；每条均为 `queued_events=1 consumed_events=1 sequence_gaps=0 queue_overflows=0 callback_panics=0`。随后 modifier 20-cycle 得到 `flags_changed=40 reconciled_releases=20 unsupported_modifier_resets=0`，mouse 20-cycle 得到 `mouse_down=20 mouse_up=20 reconciled_releases=20`；两批均 `sequence_gaps=0 sequence_duplicates_or_out_of_order=0 callback_panics=0 clean_shutdown=true`。这证明 callback 字段映射和三类候选校正闭环，不冒充物理设备或系统自然丢事件证据。
...
目前已覆盖 denied/granted、tap timeout/disable、permission revocation 和 session reset 的状态测试，以及真实 tap 创建/运行/停止、严格 100 次 tap wrapper restart 与 malloc/NSZombie 检查、受控 timeout/user-disable 恢复、公开 NSWorkspace observer 的生命周期 Reset、keyboard/modifier/mouse 真实 callback release 在 consumer 边界丢弃后的候选校正、callback panic boundary 和 queue 的 FIFO/overflow/close contract。系统自然触发的 timeout、TCC 拒绝/撤销、物理输入或系统自然丢失 release 后的校正、runtime pressed state 接入和真实锁屏/睡眠/快速用户切换恢复仍必须在受控 macOS 实机完成；100 次循环尚未覆盖 timeout/权限故障或 Instruments 级 allocation/port 采样。纯函数和 target gating 已通过本机 `x86_64-unknown-linux-gnu` `cargo check --all-targets`，Ubuntu CI 继续提供原生 Linux contract test。
