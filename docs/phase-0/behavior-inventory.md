...
| 主窗口     | 10%-500% 缩放和 10%-100% 透明度                     | P0 首发   | 设置页和右键拖动可修改；不再在托盘/右键菜单重复放置数值项 | 逻辑尺寸、Retina/DPI 和手势     |
...
| 主窗口菜单 | 打开设置、模型窗口显隐/穿透/置顶/鼠标移入隐藏、可用的检查更新和退出；托盘与模型窗口右键共用同一套菜单 | P0 首发   | 两个入口复用同一 popup 根和模型窗口分组 | 共用菜单的层级、状态与实机点击   |
...
| 托盘       | 可隐藏的托盘/菜单栏图标与动态状态菜单               | P0 首发   | 菜单按设置、模型窗口、检查更新和退出分组             | 图标隐藏后的可恢复入口          |
...
修订记录（2026-09-25）：托盘菜单与模型窗口右键菜单统一复用同一套菜单树。菜单保留设置、按模型窗口分组的
隐藏模型窗口（勾选表示已隐藏）/穿透/置顶/鼠标移入隐藏、可用的检查更新和退出；两个入口展示相同的项目、状态
和 action。源码、版本、重启以及重复的缩放/透明度选项从原生菜单移除，分别由 About、设置页和右键拖动承担；
详见 ADR-0068。
