...
状态层只保留一个 `BTreeSet<InputKey>`：重复按下不会复制状态；正常 `Up`、校正快照中缺失的 key 和 `Reset` 都能释放 pressed state。计数器只记录事件类别，不记录真实键值。
...
测试覆盖：issue #47 中 `Ctrl+Alt+A` 的 A-up 丢失恢复、仍按住的键不被误清除、重复 down 计数以及 session reset。它证明的是状态层行为，不能替代 Windows Raw Input、`GetAsyncKeyState` 或 macOS `CGEventSourceKeyState` 实机验证。
