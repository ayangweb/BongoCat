...
后重新显示——会作为**一个巨大的步长**到达：一次性 motion 直接判定播完并被清除、expression 淡出
瞬间结束、motion user data 事件被跳过（`skipped_occurrences` 已有计数）。一个自然的想法是给单帧
步长设上限（例如 `100 ms`），让动画「接着演」。
...
一次性 motion 已完成、淡出已结束，因此清除 active motion 并回到 idle 是正确的瞬时状态，不是被
「跳过」。参考实现（无上限的 `LAppPal` 差值）行为完全一致。因此这里记录的是**评估后维持现状**，
不是待办。
