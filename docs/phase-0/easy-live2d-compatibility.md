...
3. motion 使用 normal priority，读取 motion3/model3 fade 与 effect ID；产品触发只推进一个循环，完成后保持终点 motion layer，直到替换、显式停止或模型切换。开始新的带声音 motion 时，旧 motion voice 被停止，同一时刻最多一个 motion voice。
