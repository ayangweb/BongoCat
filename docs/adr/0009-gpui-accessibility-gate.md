...
- macOS `.app` 的延迟 runtime probe 已通过 AX value/invalid 状态观察到 `Runtime Ready · revision 1` -> `runtime probe failed` -> `Runtime Ready · revision 2`；Windows UI Automation 的 busy/error/retry 门禁等待本批 CI 结果，VoiceOver/Narrator 宣读仍未验证；
