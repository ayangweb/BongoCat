...
- macOS `.app` 的延迟 runtime probe 已通过 AX value/invalid 状态观察到 `Runtime Ready · revision 1` -> `runtime probe failed` -> `Runtime Ready · revision 2`；commit `ea08d3d` 的 push job `99203157592` 已从 Windows UI Automation 读到 loading value，但 runner 托管 UIA client 不提供 `AriaPropertiesProperty` 标识，无法据此验证 AccessKit 的 `busy=true` 投影；error/retry 仍等待后续 CI，VoiceOver/Narrator 宣读仍未验证；
