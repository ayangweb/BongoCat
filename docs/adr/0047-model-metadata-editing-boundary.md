...
后仍走既有的「选择文件夹 / 选择 .zip」流程（含导入前的标题输入）；其余每张卡片是
`cover.png` + 标题 + 可用性 + 操作行（选中 / 打开模型位置 / 编辑 / 删除，删除前弹出确认浮层）。
...
- **不做拖拽导入**：旧版导入区可以拖入文件，本次仍是「选择文件夹 / 选择 .zip」按钮，因为 GPUI
  侧没有可用且已注册的文件拖放通道，本次不新增该通道。
