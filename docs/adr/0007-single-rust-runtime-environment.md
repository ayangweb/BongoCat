...
历史实现仍需要在迁移期提供行为观察、配置样本和资源兼容证据，但不应进入 Native Rewrite 的生产执行链路。
...
不得引入第二套常驻业务运行环境、跨语言共享业务 core、sidecar 业务进程或用于同步核心状态的 IPC 边界。历史应用只作为迁移输入和行为对照，不随 Native Rewrite 生产应用启动，也不作为功能 fallback。
...
- 迁移完成前保留历史源码、tag 和样本；发布切换时从生产包移除历史 runtime 和 Web 资产。
