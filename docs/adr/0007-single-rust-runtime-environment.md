...
历史实现仍用于行为观察和模型资源兼容证据，但不应进入 BongoCat 的生产执行链路。旧配置只作为已归档考古样本，不属于产品输入。
...
不得引入第二套常驻业务运行环境、跨语言共享业务 core、sidecar 业务进程或用于同步核心状态的 IPC 边界。历史应用只作为行为与资源对照，不随 BongoCat 生产应用启动，也不作为功能 fallback。
