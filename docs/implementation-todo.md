...
- [ ] cursor/gamepad axis 使用 latest-value 合并通道。
  ...
  - 状态（2026-09-25）：共享 `gilrs_gamepad` adapter 固定 fork commit
    ...
    WGI 无焦点窗口投递、物理设备与长期 backlog 仍未实测；
...
- [ ] 队列溢出必须计数、记录并触发安全恢复。
  ...
  - 状态（2026-08-30，历史基线）：产品 `InputProducer` 已聚合 enqueued、queue full、overflow 后
    ...
    原因且只计一次。macOS 正式 callback 已改用该 producer；Windows 正式 producer 已接入；backend event queue 的上界、overflow marker、reset epoch 和 bounded stop/join 已由 fork 提供；长期 backlog 与异常路径仍待物理/系统验证。
...
- [ ] 定义手柄按钮、axis、trigger、dead-zone 和断开复位。
  - 状态（2026-08-31）：正式 runtime 已接入带 device generation 的 16 个标准手柄按钮、可靠
    ...
    持久化 stick/trigger dead-zone。平台采集已迁入 gilrs 私有 adapter，fork 生命周期、bounded queue/epoch、authoritative reset 与 shutdown acknowledgement 已通过代码验证，
    实机验证仍待完成。
...
- [ ] gilrs IOHID 设备和 profile 映射进入统一事件。
  - 状态（2026-09-25）：fork 内置 SDL mapping、IOHID device discovery、D-pad filter 和项目
    ...
    controller/profile、热插拔、lost-release、100-cycle 与长期资源证据尚未取得，因此保持未勾选。
...
- [ ] FFI 错误映射为稳定 Rust error code。
  ...
  - 状态（2026-09-06）：`bongocat-runtime` 新增集中式 Live2D error boundary；模型加载、参数/
    ...
    不向 UI 泄漏 FFI detail、路径或裸类型。16 个当前 Core code 的映射回归已通过；精确
    Core code 的 UI 诊断投影和跨平台完整错误矩阵仍待完成，
    因此本项保持未勾选。
...
- [x] 更新窗口覆盖检查、下载进度、校验、安装与失败重试。
  ...
  - 补充（2026-09-26，ADR-0035 补充节）：窗口改为**按内容定高**，夹在下限 180 与上限 460（与改版前
    同一高度）之间。实测四个状态的内容只有 159–187px，固定 460px 让「检查更新」这条最常见路径全程
    显示一个大半是空的大框。说明区按内容取高并以常量封顶，使测量不与窗口高度互相喂给；内容列
    `flex_shrink_0`，使窗口偏小时帧能报告「放不下」而不是静默压缩说明区；间距与内边距取自该帧自身的
    bounds。`update_window` 的无头渲染测试由 7 个增加到 13 个，另加 3 个纯函数测试；macOS 实机截图
    已确认紧凑状态无裁剪。Windows 缩放、800x600 与 125–200% 缩放仍未验证，详见 ADR-0035 的残余风险。
  - 补充（2026-09-26，ADR-0035 补充节）：修掉开窗与状态切换时「先跳到上限高度再回落」的闪烁，并把
    「用户拖动后由用户高度接管」换成**窗口不允许拖拽改变尺寸**。前者：删掉过矮时先跳上限的分支（内容
    列不收缩，测量在窗口装不下时依然准确），并改为隐藏状态下量好目标高度、`resize` 后排队重画再显示
    ——macOS 的 `resize` 是异步的，且窗口未显示时拿不到帧，所以「等画对了再显示」必须自己排任务而不是
    等 frame loop。后者：`is_resizable: false`（macOS 去掉 `NSWindowStyleMaskResizable`，Windows 去掉
    `WS_THICKFRAME`/`WS_MAXIMIZEBOX`），理由与定高同源：拖拽选的高度会被下一次状态变化撤销，于是
    `pinned` 状态与 `start_height_pinning` 一并删除。平台把内容高度取整到整像素，故目标高度判定允许
    半像素误差。实机 trace：开窗 `180 -> 187` 且在 187 才显示，状态切换 `187 -> 410.5` / `257.5` / `207`
    均一步到位，上限 460 从未成为目标高度；辅助功能读数显示更新窗口无 zoom 按钮。渲染测试 13 -> 20 个。
    **未验证**：Windows 的开窗时序（`resize` 是否同样异步、隐藏窗口是否同样无帧）、800x600 与
    125–200% 缩放。
  - 补充（2026-09-26，ADR-0035 补充节，已删除）：定高期间临时新增过 `bongocat_ui::update_state_browser`
    与 `--update-window-states`，用来人工过一遍十个状态——Development 构建在发请求前就被
    `unavailability()` 拦下，因此除 `Unavailable` 外所有状态在构建机上原本都看不到。定高与闪烁改完、
    无头渲染测试逐个状态覆盖之后即删除：它只剩人工过一遍的用途，而无头测试覆盖更多（每个状态的动作、
    高度、内容），留着等于让发布构建多带一个没人维护的界面。当时的 9 个测试与实机截图（紧凑 187px、
    `Installing` + 完整更新内容 431px）是定高结论的实机依据，已记入 ADR-0035。随它一同失去且从未
    验证的：Windows 上两个窗口的并排位置、面板键盘导航。
  - 补充（2026-09-26，ADR-0035 补充节）：修掉「打开更新窗口时先显示上次检查结果、过一会儿才出现进度条」
    的闪烁。原因是请求发得太晚：系统菜单与 About 页的入口都走 `open_update_window_and_check`，而它先
    开窗显示、再通过 view 发 `Check`；命令到 worker 线程后才发布 `Checking`，窗口又按 250 ms 轮询共享
    状态。现在窗口带着 `UpdateWindowStart::{Current, Check}` 打开，`Check` 在 view 构造期间（第一帧绘制
    之前）就发出请求；请求之后由 view 记住当时的 revision 并渲染 `Checking`，`poll_state` 在 revision
    未前进时不采用仍属于上一次检查的阶段，命令发不出去时不渲染进度条。准入条件
    `asks_for_a_new_check` 固定三条：没有未完成的请求、当前阶段不忙、构建不是 `Unavailable`——最后一条
    是必需条件而非防御，因为这类构建的 worker 只重新发布已有阶段（revision 不前进），进度条会永远转
    下去。锚点取的是发请求那一刻从共享状态读到的 revision，而不是窗口上次轮询的那个：结果可能落在两次
    轮询之间，用旧锚点会让下一次轮询把那个还没看到的结果当成对本次请求的回答。已打开的窗口仍由
    `view.check` 请求，刚创建的窗口那次调用是空操作。渲染测试 20 -> 23（2 个纯函数 + 3 个无头渲染），
    `bongocat-ui` 170 项通过；三个渲染断言分别去掉「构造期发请求」「轮询不采用未前进 revision」「锚点取
    请求时的 revision」时都会失败。**未验证**：实机第一帧的观感，本次未启动产品人工核对。
...
- [ ] gilrs/IOHID 连接、断开和不同 profile。
  - 状态（2026-09-25）：旧 GameController spike 已删除；当前 fork revision 已提供 IOHID callback
    ownership、bounded stop/join、queue/reset epoch 和 authoritative snapshot。100-cycle restart、
    TCC/系统生命周期和物理 Xbox/DualSense/Switch/HID profile 热插拔矩阵仍待目标 macOS 实机执行。
...
- [ ] 1000 次显示/隐藏、穿透和置顶切换。
- [ ] 100 次模型切换和损坏模型恢复。
- [ ] 100 次 GPUI 设置窗口关闭/重开，并验证每次销毁后重建及一级侧边栏页面记忆；不得把同一进程内页面记忆误记为窗口复用。
- [ ] 100 次输入服务 restart。
- [ ] 8 小时固定模型 soak。
- [ ] 8 小时活跃输入/模型轮换 soak。
...
17. [ ] `P1-SETTINGS-WINDOW-LIFECYCLE`：设置窗口关闭后保持后台产品运行，并可销毁/重建设置窗口。
        - 依赖：正式 GPUI 设置窗口、app coordinator、runtime/render owner。- 退出条件：window close 不触发 shutdown；两个平台 close 都销毁当前 GPUI
        窗口，下一次 open 创建新 Entity 并从当前 revisioned snapshot 刷新；process-local
        `SettingsNavigationMemory` 恢复一级侧边栏页面；显式 Quit 仍按既定顺序 join 全部 owner；
        Windows/macOS release smoke 与完整 workspace 门禁通过。
        - 状态（2026-09-25，ADR-0068）：设置和更新窗口普通 close 均销毁；设置页通过
        `SettingPage::title_suffix` 回报当前一级页面，coordinator 持有 process-local memory，
        新窗口用 `Settings::default_selected_index` 恢复。纯 contract、定向 UI/app 测试已通过；
        双平台真实 close/重建/页面恢复 smoke 仍待完成，因此保持未勾选。
        - 历史证据（2026-08-31）：此前 macOS/Windows smoke 验证了永久隐藏/重显路径；该结论已由
          ADR-0068 取代。历史 Windows `WM_DESTROY` fast-fail 证据仍约束显式 Quit 的最终兼容退出，
          普通设置/更新窗口销毁需重新完成双平台原生 smoke。
        - 历史状态（2026-08-31）：Windows run `33328391234`、job `99302481796` 已证明普通 close 隐藏有效，但随后允许真实
...
29. [x] `P7-MACOS-APPLICATION-REOPEN`：通过正式 `.app` 和 LaunchServices 唤醒后台产品。
    ...
    - 退出条件：`.app` 固定 Bundle ID、最低系统和禁止多实例 metadata，内置三个预置模型且
      ...
      已销毁的设置窗口被重新创建并恢复当前 snapshot 与一级侧边栏页面，后台 frame source 持续；退出仍进入
    ...
    - 状态（2026-09-25，ADR-0068）：设置窗口生命周期改为 close 销毁、application reopen 创建新
      Entity，并由 process-local `SettingsNavigationMemory` 恢复一级侧边栏页面；此前的
      "隐藏预渲染窗口 → 重显同一 Entity" smoke 结论已过时，需按新生命周期复验。
...
45. [ ] `P2-GAMEPAD-RUNTIME`：将双平台 gilrs 手柄 producer 接入正式 runtime。
    ...
    - [x] 迁移 Reset 后的 connection/held-state 重播。
      - 状态（2026-09-25）：Windows queue/lifecycle Reset、macOS tap/session/callback Reset 和
        ...
        gilrs reset epoch 重新读取并发布 authoritative held button 与六轴，不分配新 generation；overflow marker 会触发同一 reset/reseed 恢复。

    - 未完成门禁（2026-09-25）：
      - Windows WGI 仍需在 hidden/unfocused window、click-through、启动已连接、重连和多手柄
        状态下完成 Windows 10/11 实机矩阵；无设备 smoke 不能替代。
      - macOS IOHID 仍需 TCC deny/grant/revoke、自然 timeout、锁屏/睡眠/快速用户切换、真实
...
96. [x] `P1-LOGIN-STARTUP-DEVELOPMENT-GATE`：开发构建不提供登录时启动，设置项整行禁用并保留可见说明。
    ...
    - 事实依据（改正前核实，未重新猜 API）：
      ① 契约里早已有 `SettingsStartupItemUnsupportedReason::BuildEnvironment` 与
        ...
        后端从不产生该变体（第 82 项已如实记录为"保留为契约变体、无生产者"），因此开发构建实际会写
        `BongoCat Development` 登录项。
      ② 登录项注册的是 `current_exe()`，而开发构建的可执行文件是 Cargo 构建产物：`cargo build`、
    ...
    - 验证（2026-09-20，本机 macOS / aarch64）：
      ...
      通过；`cargo test --locked --workspace` 全绿（`bongocat-app` 138、`bongocat-ui` 118、
      `bongocat-i18n` 10，0 failed）；`cargo check --locked --workspace --release` 通过；
      `tools/validate-{fixtures,json-schema,locales}.py` 与 `tools/tests`（63 用例）通过。
      新增/更新的定向断言：
      ① `bongocat-app::login_startup_is_gated_on_the_build_environment`——门禁等于
        `BUILD_ENVIRONMENT == Production`（开发与 `--features production` 两种特性下都成立）；
