...
- 状态（2026-08-30）：`spikes/input-macos/` 已建立 macOS 权限/tap 生命周期 contract、listen-only `CGEventTap` 专用 run loop、panic-isolated callback、固定容量 callback queue 和候选 pressed-set 周期校正；callback edge/Reset 携带单调 sequence，严格 cycle validator 要求无 gap/duplicate 且 queued/consumed/discarded 完整守恒。timeout/user-disable、NSWorkspace lifecycle 与 sequence 变更后的 private `CGEventSource` release-loss 20/20 cycle 均通过，后者捕获 20 组 down/up、故意丢弃 20 个 release，再由状态校正全部清零，gap/overflow/panic 为 0。物理输入/系统自然丢事件、真实 modifier/鼠标字段、系统自然 timeout、TCC 拒绝/撤销和真实锁屏/睡眠/快速用户切换恢复仍未完成，详见 `docs/phase-0/input-macos-spike.md`。
