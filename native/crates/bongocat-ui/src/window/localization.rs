...
pub(super) fn text(language: SettingsLanguage, key: UiText) -> &'static str {
    let values = match key {
        ...
        UiText::ThirdPartyLicensesDescription => [
            ...
            "第三方 Rust 依赖遵循各自的许可证声明，并按照项目认可的许可证策略进行核查。",
        ],
        ...
        UiText::CubismAttributionDescription => [
            ...
            "使用 Live2D Cubism SDK for Native 5-r.5（Core 06.00.0001）。Cubism Core 采用 Live2D 专有软件许可证。版权所有 Live2D。公开再分发和最终署名仍须经 Live2D 批准及发布审核。",
        ],
        ...
        UiText::PrivacyDescription => [
            ...
            "输入在本地处理。诊断信息和日志不包含按键序列、剪贴板内容、用户文件内容或密钥。",
        ],
        UiText::ModelCatalog => ["Model Catalog", "模型目录"],
        ...
        UiText::ModelBehaviors => ["Model behaviors", "模型行为"],
        UiText::NoModelBehaviors => ["No declared behaviors", "没有声明的行为"],
        UiText::Motion => ["Motion", "动作"],
        UiText::Expression => ["Expression", "表情"],
        UiText::Preview => ["Preview", "预览"],
        UiText::PreviewingBehavior => ["Previewing behavior...", "正在预览行为..."],
        ...
        UiText::RuntimeAndInput => ["Runtime and Input", "运行时与输入"],
        UiText::RuntimeDiagnostics => ["Runtime Diagnostics", "运行时诊断"],
        ...
        UiText::PressRecordShortcut => ["Press to record shortcut", "按下录制快捷键"],
        UiText::ClickRecordShortcut => ["Click to record shortcut", "点击录制快捷键"],
        UiText::PressKey => ["Press key", "请按键"],
        UiText::Capture => ["Capture", "录入"],
        UiText::Clear => ["Clear", "清除"],
        UiText::NotSet => ["Not set", "未设置"],
        UiText::InputService => ["Input service", "输入服务"],
        UiText::InputMonitoring => ["Input Monitoring", "输入监控"],
        ...
        UiText::Granted => ["Granted", "已授权"],
        UiText::Unsupported => ["Unsupported", "不支持"],
        ...
        UiText::ReleaseFallbackTimeoutDescription => [
            ...
            "仅在常规释放和状态校正均失败时，才释放已捕获的键盘按键。设为 0 可禁用。",
        ],
        ...
        UiText::GamepadTriggerDeadZoneDescription => {
            ["Ignore small trigger movement.", "忽略轻微的扳机移动。"]
        }
