...
- The settings sidebar is now organised by area instead of stacking every app-level preference on the General page. Four pages that used to be groups on General now stand on their own next to General, Models, Shortcuts, and About: Overlay holds the model window's behaviour, appearance, and frame-rate limit; Interaction holds how the model reacts to behaviour shortcuts and to the pointer; Input holds the keyboard release fallback and the gamepad dead zones; and Application holds runtime status, system integration, and startup and updates. A page that still holds more than one concern shows those concerns as a second level in the sidebar, so one click reaches them. General now only holds appearance.
...
- Each top-level page in the settings sidebar — General, Models, Overlay, Interaction, Input, Shortcuts, Application, and About — now carries its own icon, so the pages can be told apart at a glance.
