...
开始编写前先理解需求和受影响的现有流程，再按以下顺序选择实现方式，停在第一个可行方案：

1. 这个功能是否真的需要构建；YAGNI，能通过现有产品行为或配置满足时不新增能力。
2. 仓库中是否已有可复用的 helper、contract、类型或模式。
3. Rust 标准库是否已经覆盖需求。
4. 操作系统原生能力是否已经覆盖需求。
5. 当前 workspace 已安装的依赖是否已经覆盖需求。
6. 是否存在可调研并采用的成熟、可靠且维护良好的第三方库或开源方案。
7. 只有前几项都不能满足需求，或引入方案的成本明显高于自行实现时，才实现满足当前需求的最小自有代码。

第三方方案首先必须通过 §9 的版本、许可证、维护状态、平台支持、unsafe 面积和替换边界评审；
不得为了“自给自足”重复实现已经存在的通用能力，也不得为了少写少量胶水代码引入不可靠或
不成比例的依赖。复用不等同于直接泄漏第三方类型：仍须遵守本文档的架构边界和公共 API 约束。
...
5. 按 §3.1 完成实现方式评估；任何新依赖都要说明用途、维护状态、许可证和替换边界。
...
- 不实现完整设置 UI。
- 不批量迁移历史功能。
- 未经明确授权，不删除历史源码和行为对照；已授权退役时，将历史参考迁至受保护的远端分支并清理本地引用。
...
- 实现新功能前必须按 §3.1 调研现有代码、标准库、平台能力和成熟第三方方案；选择自研时，在任务
  或 ADR 中简要记录现有方案为何无法满足需求，或引入成本为何明显高于自行实现。常规、边界清晰的
  小工具调用无需为每次编码保留独立评估文档。
...
- 依赖版本审计以 Native Rewrite workspace 和离线工具为范围，不得引入历史 Tauri workspace 依赖。
...
- 系统能力先评估是否存在满足需求和边界的成熟 crate；若不存在，再使用 `windows-rs`、`objc2`
  等基础 binding 编写最小封装。不得为了减少少量代码引入不可靠抽象，也不用“减少依赖”作为
  拒绝可靠成熟方案的理由。
...
Native GPUI 设置界面统一使用 `gpui-kit = "=0.6.4"`，并以它作为唯一直接 GPUI 依赖。
...
历史源码是行为考古和模型兼容的参考输入，但在 TODO 10.3 完成后只保留于受保护的远端
`master` 与 `pre-refactor-tauri` 分支：

- 不在远端历史实现上继续扩展 Native Rewrite 功能，也不得将其重新接入当前工作树或产品依赖图。
- 读取远端历史配置和模型样本时不得原地修改。
...
- 上游原版 [MMmmmoko/Bongo-Cat-Mver](https://github.com/MMmmmoko/Bongo-Cat-Mver)
  ...
  Technical Design 和 ADR-0030 在新架构中实现；不得直接复制其 C++ 业务代码或让旧架构
  覆盖当前边界。
...
- **执行 `git push` 前必须先完成 CHANGELOG 检查，顺序不可颠倒**：先判断本次待推送变更是否需要写
  CHANGELOG，再决定更新或跳过，只有 CHANGELOG 处理完毕后才允许 push。
  - 判断依据是本次待推送范围内（`git log @{u}..HEAD` 及其 diff；无上游时用本次新增 commit 范围）
    是否存在对用户而言重要的变更：新增、移除或改变功能与默认值，面向用户的行为、性能、UI、
    平台支持、本地化、配置格式或数据影响，已知不兼容与升级注意事项。
  - 仅当存在上述用户可见变更时才更新 CHANGELOG：`CHANGELOG.md` 与 `CHANGELOG.zh-CN.md` 必须同步
    更新，沿用现有 section 分类、条目风格和语言；不确定属于哪个 section 时按最贴近用户感知的
    分类写入，不新增临时 section。
  - 变更对用户不重要时跳过更新：纯内部改动（重构、测试、CI、格式化、注释、构建脚本、内部依赖
    调整）和未落地为产品行为的中间实现都不得写入 CHANGELOG，也不得为了“每次 push 都有记录”而
    添加无信息量条目。
  - 判断结论必须写进最终报告：更新了就说明写了什么；跳过就明确写出“本次跳过 CHANGELOG”及理由，
    不得静默省略。
  - CHANGELOG 的改动必须先提交，再执行 push；不得带着未提交的 CHANGELOG 修改推送。
