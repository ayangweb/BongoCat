...
// Capturing a model's cover image.
//
// The capture is a session rather than a function because it has to be started,
// wait for a frame that is actually drawn, and then be torn down — and a
// function that could return before the frame arrived would return an empty
// image.
...
pub struct ModelCoverCaptureSession {
    ...
    pub(crate) inner: windows::CoverCaptureSession,
}
