...
const { leftPressed, rightPressed } = useGamepad()
...
watch([modelStore.pressedKeys, leftPressed, rightPressed], ([keys]) => {
  const values = Object.values(keys)

  const hasLeft = values.some(item => item.includes('left-'))
  const hasRight = values.some(item => item.includes('right-'))

  handleKeyChange(true, leftPressed.value || hasLeft)
  handleKeyChange(false, rightPressed.value || hasRight)
}, { deep: true })
